...
class TelefonosController extends AppController {
    ...
    function edit($id = null) {
        if (!$id && empty($this->data)) {
            $this->flashWarnings(__('Telefono no válido', true));
            $this->redirect($this->referer());
        }
        if (!empty($this->data)) {
            if ($this->Telefono->saveAll($this->data)) {
                $this->Session->setFlash(__('El telefono ha sido salvado correctamente', true));
                $this->redirect($this->referer());
            } else {
                $this->flashWarnings(__('El teléfono no ha podido ser salvado. Por favor, inténtelo de nuevo.', true));
            }
		}
	}		
}
