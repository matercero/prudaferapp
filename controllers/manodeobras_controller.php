...
class ManodeobrasController extends AppController {
    ...
    function edit($id = null) {
        ...
        if (!empty($this->data)) {
            if ($this->Manodeobra->save($this->data)) {
                $this->Session->setFlash(__('La Mano de Obra ha sido guardada', true));
                $this->redirect($this->referer());
            } else {
                $this->Session->setFlash(__('La Mano de Obra no ha sido guardada. Prueba de nuevo.', true));
                $this->redirect($this->referer());
            }
        }
        if (empty($this->data)) {
        ...
        $tarea = $this->Manodeobra->Tareaspresupuestocliente->find('first', array('contain' => array('Presupuestoscliente' => array('Centrostrabajo', 'Cliente')), 'conditions' => array('Tareaspresupuestocliente.id' => $this->data['Manodeobra']['tareaspresupuestocliente_id'])));
        $this->set('tareaspresupuestocliente', $tarea['Tareaspresupuestocliente']);
        if (!empty($tarea['Presupuestoscliente']['centrostrabajo_id'])) {
            $this->set('centrostrabajo', $tarea['Presupuestoscliente']['Centrostrabajo']);
        }
