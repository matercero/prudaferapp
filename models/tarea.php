...
class Tarea extends AppModel {
    ...
    var $virtualFields = array(
        'total_partes_real' => 'Tarea.totalkilometrajereal + Tarea.totaldesplazamientoreal + Tarea.total_horastrabajoprecio_real + Tarea.totaldietasreales + Tarea.totalotroserviciosreales',
        ...
        if ($tarea['Tarea']['tipo'] == 'centro') {
            foreach ($tarea['Parte'] as $partecentro) {
                if ($partecentro['id'] != $deleted_id) {
                    ...
                    $tarea['Tarea']['total_horastrabajoprecio_imputable'] += ($partecentro['horasimputables'] * $partecentro['preciohoraencentro']) * (1 - $partecentro['descuento_manodeobra'] / 100);
            ...
            foreach ($tarea['Partestallere'] as $partetaller) {
                if ($partetaller['id'] != $deleted_id) {
                    ...
                    $tarea['Tarea']['total_horastrabajoprecio_imputable'] += ($partetaller['horasimputables'] * $partetaller['preciohoraentraller']) * (1 - $partetaller['descuento_manodeobra'] / 100);
                }
            }
        }
        /*
        ...
        if (!empty($tarea['ArticulosTarea'])) {
            foreach ($tarea['ArticulosTarea'] as $articulos_tarea) {
                if ($articulos_tarea['id'] != $deleted_id) {
                    $tarea['Tarea']['total_materiales_imputables'] += ($articulos_tarea['cantidad'] * $articulos_tarea['precio_unidad']) - (($articulos_tarea['cantidad'] * $articulos_tarea['precio_unidad']) * ($articulos_tarea['descuento'] / 100));
    ...
    function dime_siguiente_numero($ordene_id) {
        $query = 'SELECT MAX(t.numero)+1 as siguiente  FROM tareas t WHERE t.ordene_id = "' . $ordene_id . '"';
