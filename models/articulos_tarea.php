...
class ArticulosTarea extends AppModel {
    ...
    function beforeSave($options) {
        ...
        if (!empty($this->data['ArticulosTarea']['id'])) {
            ...
            $cantidad = $articulos_tarea['ArticulosTarea']['cantidadreal'] - $this->data['ArticulosTarea']['cantidadreal'];
            ...
            $articulo = $this->Articulo->find('first', array('contain' => '', 'conditions' => array('Articulo.id' => $this->data['ArticulosTarea']['articulo_id'])));
            $this->Articulo->id = $this->data['ArticulosTarea']['articulo_id'];
            $existencias = $articulo['Articulo']['existencias'] - $this->data['ArticulosTarea']['cantidadreal'];
            if ($existencias < 0) {
                $this->session_message = 'No hay existencias suficientes del Artículo'.$articulo['Articulo']['ref'].' - '.$articulo['Articulo']['nombre'];
                $guardar = False;
            } else {
    ...
    function beforeDelete() {
        ...
        $this->Articulo->saveField('existencias', $articulo['Articulo']['existencias'] + $articulos_tarea['ArticulosTarea']['cantidadreal']);
