<fieldset>
    ...
    <legend><?php __('IMPUTACIÓN A LA ORDEN'); ?><?php echo (!empty($ordene)) ? ' ' . zerofill($ordene['Ordene']['numero']) : '' ?></legend>
    <h2><?php __('Pedido cliente ' . $pedidoscliente['Pedidoscliente']['serie'].'-' . zerofill($pedidoscliente['Pedidoscliente']['numero'])); ?></h2>
    <dl>
        <?php
        $i = 0;
        $class = ' class="altrow"';
    ...
    <?php echo $this->Form->input('estadosordene_id', array('type' => 'hidden', 'value' => 1)); ?>
    <?php
    if (empty($ordene_id)) {
        if (!empty($avisostallere)) {
            ...
            echo $this->Form->input('numero', array('type' => 'hidden', 'value' => $numero));
            echo $this->Form->input('fecha', array('dateFormat' => 'DMY'));
            echo $this->Form->input('fecha_prevista_reparacion', array('dateFormat' => 'DMY'));
            ...
            echo '<h3>Se va ha crear una Orden nueva para el Aviso que seleciones:</h3>';
            echo $this->Form->input('avisostallere_id', array('type' => 'select', 'class' => 'select_basico', 'label' => 'Número de Aviso de Taller en estado Pendiente revisión por DAFER
'));
            echo $this->Form->input('almacene_id', array('type' => 'hidden', 'value' => $pedidoscliente['Presupuestoscliente']['almacene_id']));
            echo $this->Form->input('numero', array('type' => 'hidden', 'value' => $numero));
            echo $this->Form->input('fecha', array('dateFormat' => 'DMY'));
